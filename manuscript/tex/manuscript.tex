...
Source code, the random-sample seed and morphology checklist, the synthetic check, and the test suite are available at \url{https://github.com/thomas-landry/ppg-microvascular-feasibility-mimic} (commit hash \texttt{[hash to be inserted]}), archived on Zenodo at \texttt{[Zenodo DOI to be inserted]}. The environment lockfile (\texttt{pyproject.toml}, \texttt{uv.lock}) and a reproducible figure script are included. MIMIC-IV\cite{10} and MIMIC-IV-WDB v0.1.0\cite{9} are not redistributed; PhysioNet credentialed access and an executed Data Use Agreement are required to reproduce derived feature tables.
